...
let yaw=.25,pitch=.48,drag=false,lx=0,ly=0;let penaltyGoal=null,penaltyKeeper=null,penaltyKeeperT=0;renderer.domElement.onmousedown=e=>{drag=true;lx=e.clientX;ly=e.clientY};addEventListener('mouseup',()=>drag=false);addEventListener('mousemove',e=>{if(!drag)return;yaw-=(e.clientX-lx)*.006;pitch=Math.max(.2,Math.min(1.05,pitch-(e.clientY-ly)*.004));lx=e.clientX;ly=e.clientY});
...
function startPenaltyKings(){secondGame=false;penaltyShots=0;penaltyGoals=0;penaltyTime=0;penaltyResultTimer=0;penaltyReady=true;penaltyCompleted=false;penaltyKeeperT=0;document.querySelector('#eventName').textContent='PENALTY KINGS';document.querySelector('#venue').innerHTML='<b>FOOTBALL ARENA</b><span>5 shots · beat the AI goalkeeper</span>';setRaceVisible(true);buildPenaltyArena();showToast('PENALTY KINGS! AIM FOR THE GOAL!');gameState.set('PENALTY_KINGS')}
...
function updatePenaltyKings(dt){
  penaltyTime+=dt;updatePenaltyKeeper(dt);
...
function shootPenalty(e){if(gameState.state!=='PENALTY_KINGS'||!penaltyReady||penaltyCompleted||penaltyShots>=5)return;penaltyShots++;const r=renderer.domElement.getBoundingClientRect();const x=((e.clientX-r.left)/r.width)*2-1;const y=-((e.clientY-r.top)/r.height)*2+1;const targetX=x*6,targetY=2.5+y*2.5;const keeperX=penaltyKeeper?penaltyKeeper.position.x:0;const aimQuality=Math.max(0,1-Math.hypot(targetX-keeperX,targetY-2.2)/7);const cornerBonus=Math.min(1,Math.abs(targetX)/6+.25);const goalChance=Math.max(.08,.72-cornerBonus*.28-aimQuality*.42+Math.random()*.16);if(Math.abs(targetX)<=6&&targetY>=0&&targetY<=5&&goalChance>.42){penaltyGoals++;showToast('GOAL! ⚽')}else showToast('SAVED! 🧤 GK READ YOUR SHOT!')}
