...
function renderFinalPodiumBoard(){const rows=Object.values(tournament.standings).sort((a,b)=>b.points-a.points||((b.events?.at(-1)?.score||0)-(a.events?.at(-1)?.score||0)));const b=document.querySelector('#raceBoard');if(!b)return;b.style.display='block';b.innerHTML='<b>🏆 FINAL PODIUM · COMBINED SCORE</b>'+rows.slice(0,3).map((x,i)=>{const medals=['🥇','🥈','🥉'];const g1=x.events?.find(e=>e.event==='SKY SPRINT')?.points||0;const g2=x.events?.find(e=>e.event==='TARGET MAYHEM')?.points||0;return '<div class="race-row '+(x.name==='YOU'?'you':'')+'"><span>'+medals[i]+' '+x.name+'</span><span>G1 '+g1+' + G2 '+g2+' = <b>'+x.points+'</b></span></div>'}).join('')+'<div class="race-row"><span>🏁 TOTAL</span><span>'+rows.length+' PLAYERS</span></div>'}
