...
let skyCountdown=5,returnTimer=0,resultHoldSeconds=7,elapsed=0,secondGame=false,targetTime=30,targetScore=0,targetHits=0,targetResultTimer=0,targetFlash=0,penaltyShots=0,penaltyGoals=0,penaltyTime=0,penaltyResultTimer=0,targets=[],targetMeshes=[],targetRay=new THREE.Raycaster(),mouse=new THREE.Vector2(),clock=new THREE.Clock();
...
function startPenaltyKings(){secondGame=false;penaltyShots=0;penaltyGoals=0;penaltyTime=0;penaltyResultTimer=0;document.querySelector('#eventName').textContent='PENALTY KINGS';document.querySelector('#venue').innerHTML='<b>FOOTBALL ARENA</b><span>5 shots · beat the goalkeeper</span>';setRaceVisible(true);showToast('PENALTY KINGS! 5 SHOTS!');gameState.set('PENALTY_KINGS')}
function updatePenaltyKings(dt){penaltyTime+=dt;const b=document.querySelector('#raceBoard');if(b){b.style.display='block';b.innerHTML='<b>PENALTY KINGS</b><div class="race-row"><span>SHOTS</span><span>'+penaltyShots+' / 5</span></div><div class="race-row"><span>GOALS</span><span>'+penaltyGoals+'</span></div><div class="race-row"><span>AIM</span><span>CLICK TO SHOOT</span></div>}if(penaltyShots>=5){penaltyResultTimer+=dt;if(penaltyResultTimer>2){const rows=[{name:'YOU',score:penaltyGoals},{name:'BOLT',score:2},{name:'NOVA',score:3},{name:'DASH',score:1},{name:'ROCKET',score:2},{name:'FLASH',score:4}].sort((a,b)=>b.score-a.score);tournament.addEventResult(rows);gameState.set('PENALTY_RESULTS');penaltyResultTimer=0;document.querySelector('#eventName').textContent='PENALTY KINGS RESULTS';document.querySelector('#venue').innerHTML='<b>FOOTBALL ARENA</b><span>5-shot challenge complete</span>'}}}
function shootPenalty(e){if(gameState.state!=='PENALTY_KINGS'||penaltyShots>=5)return;penaltyShots++;const x=(e.clientX/innerWidth)*2-1;const chance=.58-Math.abs(x)*.22+Math.random()*.22;if(chance>.4){penaltyGoals++;showToast('GOAL! ⚽')}else showToast('SAVED! 🧤')}
renderer.domElement.addEventListener('click',shootPenalty);
function startNextTournamentEvent(){if(tournament.complete){gameState.set('HUB');document.querySelector('#eventName').textContent='TOURNAMENT COMPLETE';document.querySelector('#venue').innerHTML='<b>OLYMPIC PODIUM</b><span>Top 3 decided by combined Game 1 + Game 2 points</span>';setRaceVisible(true);renderFinalPodiumBoard();return}document.querySelector('#raceBoard').style.display='none';if(tournament.current==='PENALTY KINGS'){startPenaltyKings();return}document.querySelector('#eventName').textContent=tournament.current;document.querySelector('#venue').innerHTML='<b>OLYMPIC PLAZA</b><span>Round '+tournament.round+' / '+tournament.totalRounds+'</span>';skyCountdown=3;gameState.set('SKY_COUNTDOWN')}
