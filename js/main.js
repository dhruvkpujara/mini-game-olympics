...
function startTargetMayhem(){secondGame=true;targetResultTimer=0;targetTime=30;targetScore=0;targetHits=0;targetFlash=0;clearTargets();player.position.set(0,.1,48);player.rotation.y=Math.PI;document.querySelector('#eventName').textContent='TARGET MAYHEM';document.querySelector('#venue').innerHTML='<b>CHALLENGE ARENA</b><span>Click targets to score points</span>';setRaceVisible(false);for(let i=0;i<9;i++){const g=new THREE.Group(),pole=new THREE.Mesh(new THREE.CylinderGeometry(.12,.12,4,8),new THREE.MeshStandardMaterial({color:0x263746})),back=new THREE.Mesh(new THREE.CylinderGeometry(2.35,2.35,.2,24),new THREE.MeshStandardMaterial({color:0x182432})),target=new THREE.Mesh(new THREE.CylinderGeometry(2,2,.35,24),new THREE.MeshStandardMaterial({color:i%2?0xe74c3c:0xf5c542})),ring=new THREE.Mesh(new THREE.TorusGeometry(1.25,.16,8,24),new THREE.MeshStandardMaterial({color:0xffffff,emissive:0x555555,emissiveIntensity:.35}));pole.position.y=2;back.position.y=4;target.position.y=4;ring.position.y=4;target.rotation.x=Math.PI/2;back.rotation.x=Math.PI/2;ring.rotation.x=Math.PI/2;target.userData.hitTarget=true;g.add(pole,back,target,ring);g.position.set((i%3-1)*10,0,25-Math.floor(i/3)*10);g.userData={target:true,phase:i};scene.add(g);targets.push(g);targetMeshes.push(target)}showToast('TARGET MAYHEM! CLICK THE TARGETS!')}
...
function shootTarget(e){if(!secondGame)return;const r=renderer.domElement.getBoundingClientRect();mouse.x=((e.clientX-r.left)/r.width)*2-1;mouse.y=-((e.clientY-r.top)/r.height)*2+1;targetRay.setFromCamera(mouse,camera);const hits=targetRay.intersectObjects(targetMeshes,false);if(hits.length){const target=hits[0].object;if(target.visible){target.visible=false;targetScore+=100;targetHits++;targetFlash=.25;showToast('🎯 HIT! +100 · SCORE '+targetScore)}}else{targetFlash=.08;showToast('MISS')}}}
